...
1. Explain how to build stateful class components.
    ...
    stateful class components must have a a constructor with super() and this.state = whatever. In newer versions of React, you can just call this.state = whatever without a constructor and super().

2. Describe the different phases of the component lifecycle.

    The component reads the constructor and states, then renders the jsx, then if any changes to state it will update state then re-render.

    Load > State > Render > componentDidMount() > Update State > Render > componentDidUpdate() > ComponentDidUnmount()

3. Demonstrate an understanding of class component lifecycle methods.

    ComponentDidMount() takes place once the component and state are rendered into the page and finishing with componentDidMount.
    ComponentDidUpdate() takes place when a change in state happens, it looks at the change then re-renders the elements finishing with componentDidUpdate
    ComponentDidUnmount() takes place when the component is be removed from the page or more probably when leaving the browser page.

4. Define stateful logic.

    stateful logic is a hook that functions with state being passed to it to perform a specific set of steps depending on the state value.

5. Describe how to test a React component with React Testing Library.

    having a file Form.js, you create a file named Form.test.js, there you first import the same imports from the Form.js, then you call either describe or test... 

    ```js
      describe("fill out the form", () => {
        //ARRANGE, ACT, ASSERT
        it("adds value to name input", () => {
          const { getByTestId } = render(<Form />);

          getByTestId("nameInput").value = "Mark";

          expect(getByTestId("nameInput").value).toBe("Mark");
        })
